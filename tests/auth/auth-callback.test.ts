...
describe("auth callback URLs", () => {
  ...
  test("rejects external and unknown next destinations", () => {
    assert.equal(getSafeAuthNextPath("https://evil.example/path"), "/signup/approval");
    assert.equal(getSafeAuthNextPath("/admin"), "/signup/approval");
  });
  ...
  test("does not redirect ordinary signup page requests", () => {
    assert.equal(
      getCorrectedAuthCallbackUrl(new URL("https://example.com/signup/approval")),
      null,
    );
  });
});